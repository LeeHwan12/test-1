...
public class TalentRepositoryImpl implements TalentRepository {
    ...
    public List<TalentDTO> TalentByMemberId(String memberId) {
        String sql = "SELECT t.*, m.username FROM talent t JOIN member m ON t.member_id = m.member_id WHERE t.member_id = ? ORDER BY t.created_at DESC";
        return template.query(sql, new TalentRowMapper(), memberId);
    }

    // 단건 조회(Read)
    @Override
    public TalentDTO readone(int id) {
        String sql = "SELECT t.*, m.username FROM talent t JOIN member m ON t.member_id = m.member_id WHERE t.talent_id = ?";
    ...
    public List<TalentDTO> readAll() {
        String sql = "SELECT t.*, m.username FROM talent t JOIN member m ON t.member_id = m.member_id ORDER BY t.created_at DESC";
    ...
    public List<TalentDTO> TalentByCategory(String category) {
        String sql = "SELECT t.*, m.username FROM talent t JOIN member m ON t.member_id = m.member_id WHERE t.category = ?";
    ...
    public List<TalentDTO> searchTalent(String keyword) {
        String sql = "SELECT t.*, m.username FROM talent t JOIN member m ON t.member_id = m.member_id WHERE t.title LIKE ? OR t.description LIKE ?";
    ...
    public List<TalentDTO> readPaged(int offset, int size) {
        String sql = "SELECT t.*, m.username FROM talent t JOIN member m ON t.member_id = m.member_id ORDER BY t.created_at DESC LIMIT ? OFFSET ?";
    ...
    public List<TalentDTO> readPagedCategory(String category, int page, int size) {
        String sql = "SELECT t.*, m.username FROM talent t JOIN member m ON t.member_id = m.member_id WHERE t.category = ? ORDER BY t.created_at DESC LIMIT ?, ?";
    ...
    public List<TalentDTO> searchPagedTalent(String keyword, int page, int size) {
        String sql = "SELECT t.*, m.username FROM talent t JOIN member m ON t.member_id = m.member_id WHERE t.title LIKE ? OR t.description LIKE ? ORDER BY t.created_at DESC LIMIT ?, ?";
        String like = "%" + keyword + "%";
        int offset = (page - 1) * size;
